...
# Отрисовка всех полигонов модели с базовым освещением
print("Отрисовка полигонов с базовым освещением...")
    ...
    if 0 <= face[0] < len(points) and 0 <= face[1] < len(points) and 0 <= face[2] < len(points):
        ...
        if cosine < 0:
            ...
            if len(screen_points) >= 3:
                # Вычисляем цвет пропорциональный косинусу угла
                # color = (-255 * (n̄,l̄) / (||n̄|| * ||l̄||), 0, 0)
                red_value = -255 * cosine
...
pil_image.save('model_basic_lighting.png')
print("Изображение сохранено как 'model_basic_lighting.png'")
