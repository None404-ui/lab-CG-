...
def prepare_vertices(points: np.ndarray, rot_matrix: np.ndarray) -> np.ndarray:
    # Преобразование вершин: центрирование, поворот и проективное преобразование
    ...
    # Применяем матрицу поворота к центрированным точкам
    rotated = (rot_matrix @ centered.T).T

    # Для корректного проективного преобразования, отодвигаем модель от начала координат
    # Вычисление максимальной границы модели
    extent = (max_vals - min_vals).max()
    z_shift = 2 * extent
    shifted = rotated + np.array([0.0, 0.0, z_shift], dtype=np.float32)

    # Параметры проективного преобразования (внутренние параметры камеры)
    focal_length = 1800.0
    ax = focal_length
    ay = focal_length
    u0 = IMAGE_WIDTH / 2.0
    ...
    # Извлекаем координаты для проекции
    X = shifted[:, 0]
    Y = shifted[:, 1]
    Z = shifted[:, 2]
    ...
    # Преобразуем в экранные 2D координаты
    screen_points = np.empty((shifted.shape[0], 2), dtype=np.int32)
    screen_points[:, 0] = np.clip(np.rint(u), 0, IMAGE_WIDTH - 1)
    # Инвертируем ось Y для соответствия системе координат изображения
    screen_points[:, 1] = np.clip(
        np.rint(IMAGE_HEIGHT - v),
        0,
        IMAGE_HEIGHT - 1,
    )
